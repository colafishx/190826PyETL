...
for i in date_range(start_date, end_date):
    ...
    (driver
     .get("https://www.google.com/flights?hl=en#flt=/m/0ftkx./m/07dfk."
          +departure_date.strftime("%Y-%m-%d")+"*/m/07dfk./m/0ftkx.2019-10-08;c:TWD;e:1;sd:1;t:f"))
    ...
    data = {'dep_airline': dep_air_list,
            'dep_etd': dep_etd_list,
            'dep_eta': dep_eta_list,
            'dep_duration': dep_dur_list,
            'dep_stops': dep_stops_list,
            'dep_price': dep_price_list}
    ...
    try:
        ...
        print(departure_date.strftime("%Y-%m-%d")+'csv file done. Mission run time=', time.process_time())
        driver.close()
